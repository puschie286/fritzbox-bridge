...
let APIHolder =
{
	 ...
	 */
	Create: function( password, ip, ssl )
	{
		// noinspection JSPotentiallyInvalidConstructorUsage
		this.fritzInstance = new fritzAPI( '', password, ip, ssl );
	},
	...
	StartPolling: function( interval )
	{
		...
		{
			this.log( 'Invalid interval - cancel polling ( ' + interval + ' => ' + Value + ' )' );
			return;
		}
		...
		this.log( 'start polling with ' + Value + ' interval' );
	...
	FilterDevices: function( deviceList, functionmask )
	{
		...
		{
			this.log( 'ERROR - functionmask invalid ', functionmask );
			return [ [], [] ];
		}
	...
	{
		if( !Array.isArray( deviceList ) ) return null;
	...
	FilterProperty: function( newDevice, oldDevice, propertyList = null, outerKey = null )
	{
		...
		let PropertyList = propertyList || Object.keys( newDevice );
		...
		PropertyList.forEach( function( key )
		{
			...
			let OldValue = oldDevice[key];
			let NewValue = newDevice[key];
	...
	Poll: function()
	{
		if( this.pollActive )
		{
			this.log('Skip poll - still waiting on last poll');
			return;
		}
		...
		this.fritzInstance.getDeviceList().then( function( deviceList )
		{
			...
			for( let Driver in ObjectContainer )
			{
				...
				{
					device.Update( this.FilterDevice( DeviceList[0], device.getData().id ), DeviceList[0], DeviceList[1] );
				}.bind(this));
			}
		}.bind( this ) ).catch( function( error )
		{
			this.log( 'failed to getDeviceList: ', error );
		}.bind( this ) );
	},
