...
let APIHolder =
{
	...
	{
		// validate
		if( device === null || typeof device !== 'object' || !Array.isArray( propertyList ) )
			return {};
	...
	{
		// validate
		if( typeof oldArray !== 'object' || oldArray === null )
		{
		...
		else if( typeof newArray !== 'object' || newArray === null )
		{
		...
		let Target = [];
		let Keys = [];
		Object.assign( Keys, Object.keys( oldArray ), Object.keys( newArray ) );
	...
	Internal_Poll: function( data )
	{
		...
		Object.keys( Drivers ).forEach( function( key )
		{
			...
			if( driver.GetName() === 'Fritzbox' ) return;
			...
			driver.getDevices().forEach( function( device )
			{
				...
				device.onUpdate( this.CopyProperties( DeviceData, driver.GetFilterList() ) );
			}.bind( this ) );
		}.bind( this ) );
	},
