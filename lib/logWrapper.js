	...
	switch( ID )
	{
		case this.SIMPLE_LOG:
			this.LoggerList.push({
				...
				{
					// validate logger
					if( this.ApiRegister[this.SIMPLE_LOG] === undefined )
					{
						this.SystemLogger( 'failed to access SIMPLE_LOG, message: ' + message );
						return;
					}

					// log message to SIMPLE_LOG api /addlog/
					this.ApiRegister[this.SIMPLE_LOG].put( '/addlog/', { log: message, group: 'fritz-bridge' } )
						.then(function() {} )
